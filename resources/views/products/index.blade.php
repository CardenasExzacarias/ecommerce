...
<body class="p-8 bg-gray-100">
    ...
    <div class="p-6 bg-white rounded-lg shadow-md">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            />
            <a href="{{ route('product.create') }}" class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-600">
                Crear Producto
            </a>
        </div>
                        ...
                        <td class="px-6 py-4 text-center border-b">
                            <div class="flex justify-center space-x-2">
                                <a href="{{ route('product.show', $product->id) }}" class="text-green-500 hover:text-green-700">
                                    @svg('eva-eye', 'w-10 h-10 inline-block')
                                </a>
                                <a href="{{ url('product/' . $product->id . '/edit') }}" class="text-blue-500 hover:text-blue-700">
                                    @svg('eva-edit', 'w-10 h-10 inline-block')
                                </a>
                                <form action="{{ route('product.destroy', $product->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700">
                                        @svg('eva-trash', 'w-10 h-10 inline-block')
                                    </button>
                                </form>
                            </div>
                        </td>
